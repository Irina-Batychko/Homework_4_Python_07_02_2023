...
berry = [random.randint(1, 20) for i in range(n)]
print(f"{berry} - ягод на кустах")

sum = list()
for i in range(len(berry) - 1):
    sum.append(berry[i - 1] + berry[i] + berry[i + 1])
sum.append(berry[-2] + berry[-1] + berry[0])
print(f"Максимальное количество ягод: {max(sum)}")